...
String[] gesture_labels = { "Abierto", "Puño", "Paz" };
...
void handleGameState() {
  boolean leftValid = (leftHandGesture >= 0 && leftHandGesture <= 2);
  boolean rightValid = (rightHandGesture >= 0 && rightHandGesture <= 2);
  
  // Check for both hands peace sign
  boolean bothPeace = (leftHandGesture == 2 && rightHandGesture == 2);
  
  if (gameState == 0) {
    // INICIO: Waiting for both hands peace to start
    if (bothPeace) {
      gameState = 1; // Start flying
    ...
    // VOLANDO: Check for pause or apply controls
    if (bothPeace && !waitingForGestureRelease) {
      gameState = 2; // Pause
      waitingForGestureRelease = true;
      controlStatus = "PAUSADO";
    } else {
      // Release the gesture lock once peace is released
      if (!bothPeace && waitingForGestureRelease) {
        waitingForGestureRelease = false;
      }
      
      // Apply normal flight controls
      if (!waitingForGestureRelease) {
        applyTwoHandControl();
      }
    }
  } else if (gameState == 2) {
    // PAUSADO: Waiting for both hands peace to resume
    if (bothPeace && !waitingForGestureRelease) {
      gameState = 1; // Resume flying
      waitingForGestureRelease = true;
...
void applyTwoHandControl() {
  ...
  // ONE hand peace sign - MAINTAIN STRAIGHT FLIGHT
  if ((leftHandGesture == 2 && rightHandGesture != 2) || 
      (rightHandGesture == 2 && leftHandGesture != 2)) {
    controlStatus = "Vuelo recto (Una mano en paz ✌️)";
    return;
  }
...
void drawStartScreen() {
  ...
  text("Derecha abierta + Izquierda puño = Giro derecha", width/2, height/2 + 200);
  text("Izquierda abierta + Derecha puño = Giro izquierda", width/2, height/2 + 225);
...
// ===== Gesture classifier - ABIERTO, PUÑO Y PAZ =====
